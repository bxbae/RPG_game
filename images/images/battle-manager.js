...
class BattleManager {
  ...
  applyStatus(target, type, turns, game, targetName) {
    if (!target.status) target.status = {
      poison: 0, stun: 0, burn: 0,
      freeze: 0, bleed: 0, weaken: 0,  // ★
    };
  ...
  processPlayerStatus(game) {
    ...
    // ★ 출혈 — 매턴 최대 HP 3% 피해
    if ((p.status.bleed || 0) > 0) {
      const dmg = Math.max(1, Math.floor((p.maxHp + p.bonusHp) * 0.03));
      p.hp = Math.max(1, p.hp - dmg);
      p.status.bleed--;
      game.log(`🩸 출혈 피해 ${dmg} (남은 ${p.status.bleed}턴)`);
    }
    // ★ 빙결 — 턴 감소 (행동불능은 attack()에서 처리)
    if ((p.status.freeze || 0) > 0) {
      p.status.freeze--;
      game.log(`❄ 빙결 중... (남은 ${p.status.freeze}턴)`);
    }
    // ★ 약화 — 턴 감소 (ATK 감소는 totalAttack에 반영)
    if ((p.status.weaken || 0) > 0) {
      p.status.weaken--;
      game.log(`💔 약화 중... (남은 ${p.status.weaken}턴)`);
    }
  ...
  processMonsterStatus(game) {
    ...
    // ★ 출혈 — 매턴 최대 HP 3% 피해
    if ((m.status.bleed || 0) > 0) {
      const dmg = Math.max(3, Math.floor(m.maxHp * 0.03));
      m.hp = Math.max(0, m.hp - dmg);
      m.status.bleed--;
      game.log(`🩸 ${m.name} 출혈 피해 ${dmg}`);
      game.battleScene?.hitFlash();
    }
    // ★ 빙결 — 반격 스킵은 _counter에서 처리, 여기선 턴 감소만
    if ((m.status.freeze || 0) > 0) {
      // 감소는 _counter 종료 후 처리하므로 여기서는 로그만
      game.log(`❄ ${m.name} 빙결 상태 (${m.status.freeze}턴)`);
    }
    // ★ 약화 — 턴 감소 (ATK 감소는 _counter에서 반영)
    if ((m.status.weaken || 0) > 0) {
      m.status.weaken--;
      game.log(`💔 ${m.name} 약화 중...`);
    }
  ...
  attack(game) {
    ...
    if ((p.status?.freeze || 0) > 0) {
      p.status.freeze--;
      game.log(`❄ 빙결! 행동 불가 (남은 ${p.status.freeze}턴)`);
      game.battleScene?.updateStatusIcons();
      this._counter(game);
      game.battleScene?.render();
      return;
    ...
    // ★ 제단 버프 턴 감소
    if ((p._altarBuffTurns || 0) > 0) {
      p._altarBuffTurns--;
      if (p._altarBuffTurns <= 0) {
        p._altarBuff = 0;
        game.log("🔮 제단 버프가 종료되었습니다.");
      }
    }
  ...
  heroUltimate(game) {
    ...
    switch (p.type) {
      ...
      case "archer":
        ...
        break;
      case "tanker":
        dmg = Math.floor(p.totalAttack * 2.8);
        m.hp -= dmg;
        p.guardBuff = (p.guardBuff || 0) + 3;
        game.log(`🛡 철벽 카운터! ${dmg} — 3턴 가드 발동!`);
        break;
    }
    game.battleScene?.showDamage(dmg, "monster");
    if (m.hp <= 0) {
  ...
  jobSkill(game) {
    ...
    const hasSkill = {
      knight:   p.activeSkills.whirlwind,
      magician: p.activeSkills.magicBall,
      archer:   p.activeSkills.rapidShot,
      tanker:   p.activeSkills.ironWall,
    ...
    switch (p.type) {
      ...
      case "tanker":
        ...
        m.hp -= dmg;
        p.guardBuff = (p.guardBuff || 0) + 2;
        game.log(`🛡 방패 강타! ${dmg} — 2턴 가드!`);
        break;
    }
    game.battleScene?.showDamage(dmg, "monster");
  ...
  _tryInflictMonster(game) {
    ...
    // ★ 빙결 — 탱커 (방패로 적을 얼림)
    if (p.type === "tanker" && Math.random() < 0.2) {
      this.applyStatus(m, "freeze", 1, game, m.name);
      game.log(`❄ ${m.name} 빙결!`);
    }
    // ★ 출혈 — 기사 (강력한 검격)
    if ((p.type === "knight" || p.type === "warrior") && Math.random() < 0.2)
      this.applyStatus(m, "bleed", 2, game, m.name);
    // ★ 약화 — 궁수 (독 화살 특수효과)
    if (p.type === "archer" && (p.passiveSkills?.poison_tip || 0) >= 2
        && Math.random() < 0.2) {
      this.applyStatus(m, "weaken", 2, game, m.name);
      game.log(`💔 ${m.name} 약화!`);
    }
  }
  ...
  _tryInflictPlayer(game) {
    ...
    // ★ 빙결 — 심연 마법사, 아비스 계열
    if ((m.id?.includes("abyss_mage") || m.id?.includes("abyss_guardian"))
        && Math.random() < 0.2)
      this.applyStatus(p, "freeze", 1, game, "플레이어");

    // ★ 출혈 — 심연 악마, 어드버서리
    if ((m.id?.includes("abyss_demon") || m.id?.includes("abyss_adversary"))
        && Math.random() < 0.25)
      this.applyStatus(p, "bleed", 3, game, "플레이어");

    // ★ 약화 — 오크 도발, 심연 지배자
    if ((m.name?.includes("도발") || m.id?.includes("abyss_overlord"))
        && Math.random() < 0.3)
      this.applyStatus(p, "weaken", 2, game, "플레이어");
  }
  ...
  _counter(game) {
    ...
    const provokeLv = p.passiveSkills?.provoke   || 0;

    // 전쟁의 함성 (기사)
    if (warCryLv > 0)
      monsterAtk = Math.floor(monsterAtk * (1 - [0,0.1,0.18,0.28][warCryLv]));

    // 도발 (탱커)
    if (provokeLv > 0)
      monsterAtk = Math.floor(monsterAtk * (1 - [0,0.12,0.20,0.30][provokeLv]));

    // ★ 약화 — 몬스터 ATK 25% 감소
    if ((p.status?.weaken || 0) > 0)
      monsterAtk = Math.floor(monsterAtk * 0.75);

    // ★ 몬스터 빙결 시 반격 스킵 + 턴 감소
    if ((m.status?.freeze || 0) > 0) {
      m.status.freeze--;
      game.log(`❄ ${m.name} 빙결로 반격 불가! (남은 ${m.status.freeze}턴)`);
      game.battleScene?.updateStatusIcons();
      return;
    }
    ...
    // 철벽 (탱커) — 피해 감소
    const ironWallLv = p.passiveSkills?.iron_wall || 0;
    if (ironWallLv > 0)
      dmg = Math.floor(dmg * (1 - [0,0.08,0.15,0.22][ironWallLv]));
    ...
    // 도발 (탱커) — 가드 확률 발동
    if (provokeLv > 0 && Math.random() < [0,0.05,0.10,0.18][provokeLv]) {
      dmg = Math.floor(dmg * 0.5);
      game.log(`😤 도발 가드! 피해 절반`);
    }
    ...
    // ★ 반격 (탱커) — 피격 후 즉시 반격 데미지
    const counterLv = p.passiveSkills?.counter_strike || 0;
    if (counterLv > 0 && dmg > 0) {
      const counterRate = [0, 0.10, 0.20, 0.35][counterLv];
      if (Math.random() < counterRate) {
        const counterDmg = Math.floor(p.totalAttack * 0.5);
        m.hp -= counterDmg;
        game.battleScene?.showDamage(counterDmg, "monster");
        game.log(`⚡ 반격 발동! ${counterDmg} 데미지!`);
        if (m.hp <= 0) {
          this._onMonsterDefeated(game);
          return;
        }
