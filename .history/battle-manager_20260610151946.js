...
class BattleManager {
  ...
  processMonsterStatus(game) {
    ...
    if (m.hp <= 0) { this._onMonsterDefeated(game); killed = true; }
    return killed;
  }
  ...
  attack(game) {
    ...
    if (this.processMonsterStatus(game)) { game.battleScene?.render(); return; }
    ...
    if (p.party === "dealer") dmg = Math.floor(dmg * 1.2);
    if (p.party === "healer") {
    ...
    if (p.party === "healer" && Math.random() < 0.15) { p.hp = Math.min(p.maxHp + p.bonusHp, p.hp + 80); game.log("✨ 대치유!"); }
    if (p.party === "dealer" && Math.random() < 0.15) { dmg *= 2; game.log("⚔ 암살!"); }
    if (p.party === "archer" && Math.random() < 0.15) { m.hp -= dmg; game.log("🏹 관통사격!"); }
    if (p.party === "mage_party" && Math.random() < 0.15) { const ex = 80 + p.level * 3; m.hp -= ex; game.log(`☄ 메테오! ${ex}`); }
  ...
  partyUltimate(game) {
    ...
    if (p.partyHp <= 0) { game.log("💔 동료 전투 불능"); return; }
  ...
  jobSkill(game) {
    ...
    switch (p.type) {
      case "warrior":
      case "knight":   dmg = Math.floor(p.totalAttack * 2);   m.hp -= dmg;   game.log(`⚔ 회전베기! ${dmg}`); break;
      case "magician":    dmg = Math.floor(p.totalAttack * 2.2); m.hp -= dmg;   game.log(`🔮 매직볼! ${dmg}`);   break;
  ...
  _tryInflictMonster(game) {
    ...
    const m = game.currentMonster;
    if (!m) return;
    if (p.type === "magician"    && Math.random() < 0.3)  this.applyStatus(m, "burn",   3, game, m.name);
    if (p.type === "archer"  && Math.random() < 0.25) this.applyStatus(m, "poison", 3 + ([0,1,2,3][p.passiveSkills?.poison_tip||0]||0), game, m.name);
    if ((p.type === "knight" || p.type === "warrior") && Math.random() < 0.15) { this.applyStatus(m, "stun", 1, game, m.name); game.log("💫 적이 기절!"); }
  }
  ...
  _counter(game) {
    ...
    if (p.party && p.partyHp > 0) {
      ...
      if (p.partyHp <= 0) { game.log("💔 동료 전투 불능!"); p.party = null; }
    }
  ...
  _onMonsterDefeated(game) {
    ...
    if (m.isFinal && Math.random() < 0.5)
      item = normalizeItem({ name:"마왕의 검", type:"weapon", attack:80, defense:0, class:"legend", enhance:0 });
    else if (m.isBoss && Math.random() < 0.25)
      item = normalizeItem({ name:"수호자 검", type:"weapon", attack:45, defense:0, class:"legend", enhance:0 });
    else
    ...
    if (hadQuest && !qResult.cleared) {
      ...
      if (!game._returnAfterBattle) game._returnAfterBattle = { type: dungeonType };
