...
class SaveManager {
    ...
    try {
      ...
      const data = {
        ...
        player: {
          ...
          baseAttack: p.baseAttack, bonusAttack: p.bonusAttack || 0,
          money: p.money, level: p.level, exp: p.exp, nextExp: p.nextExp,
          skillPoints: p.skillPoints,
          skills: p.skills, activeSkills: p.activeSkills,
  ...
  hydrate(raw) {
    if (!raw) return null;
    const rawType = raw.type || "night";
    const type = rawType === "warrior" ? "night" : rawType;
    ...
    // 기본값 보호 (구버전 호환)
    p.partyEvents        = raw.partyEvents        || { affinity25:false, affinity50:false, affinity75:false, affinity100:false };
    ...
    if (raw.bank && typeof raw.bank === "object") {
      p.bank = {
        ...
        milestones:    Array.isArray(raw.bank.milestones) ? raw.bank.milestones : [],
      };
    } else {
...
class ItemManager {
  ...
  equip(game, index, forParty = false) {
    ...
    if (item.weaponClass && forParty && p.party) {
      ...
      if (allowed && item.weaponClass !== allowed) {
        game.log(`❌ 동료는 ${allowed}만 사용 가능`); return;
      }
    }
  ...
  remove(game, index) {
    ...
    if (allEq.some(e => sameItem(e, item))) { game.log("⚠ 장착 해제 후 삭제하세요"); return; }
    p.inventory.splice(index, 1);
  }
...
class QuestManager {
  ...
  onKill(game, monsterName) {
    ...
    if (p.questProgress >= p.quest.goal) {
      ...
      game.log(`🏆 퀘스트 완료! +${completed.rewardGold}G +${completed.rewardExp}EXP`);
      if (lvUp) game.log(`🎉 레벨 업! Lv.${p.level}`);
      p.quest = null; p.questProgress = 0;
