...
const BACK_IMAGES = {
  night:  "images/back_Night.png",
  mage:   "images/back_wizard.png",
  archer: "images/back_archer.png",
};
const COMPANION_BACK = {
  healer:     "images/back_healer.png",
  tanker:     "images/back_tanker.png",
  warrior:    "images/back_tanker.png",
  dealer:     "images/back_Night.png",
  mage_party: "images/back_wizard.png",
  archer:     "images/back_archer.png",
};
...
const CUTIN_HERO = {
  ...
  mage:    "images/wizard_skill_cut.png",
...
// video 폴더에 넣을 파일명:
//   night_ultimate.mp4  ← A_knight_in_weathered... 파일명 변경
//   mage_ultimate.mp4   ← Wizard_Skills.mp4 파일명 변경
//   archer_ultimate.mp4 ← Forest_Elf_ultimate_skill.mp4 파일명 변경
//   healer_skill.mp4    ← healer_skill.mp4 그대로
const CUTIN_HERO_VIDEO = {
  night:   "video/night_ultimate.mp4",
  warrior: "video/night_ultimate.mp4",
  mage:    "video/mage_ultimate.mp4",
  archer:  "video/archer_ultimate.mp4",
};
const CUTIN_PARTY_VIDEO = {
  healer:     "video/healer_skill.mp4",
  tanker:     "video/night_ultimate.mp4",
  warrior:    "video/night_ultimate.mp4",
  dealer:     "video/night_ultimate.mp4",
  mage_party: "video/mage_ultimate.mp4",
  archer:     "video/archer_ultimate.mp4",
};
...
<div class="poke-bottom">
  ...
  startBattle(monster) {
    ...
    if (this.el.bg) {
      if (monster.isFinal) {
        // ★ 마왕 전투 → 심연 배경
        this.el.bg.src = "images/Abyss_Dungeon.png";
        this.el.bg.style.filter = "brightness(0.7)";
      } else if (monster.isBoss) {
        this.el.bg.src = "images/던전 내부 보스방 이미지.png";
        this.el.bg.style.filter = "";
      } else {
    ...
    if (this.el.monsterImg && monster.img) {
      ...
      if (this.el.monsterWrap) {
        ...
        this.el.monsterWrap.classList.toggle("is-final-boss", !!monster.isFinal);
        this.el.monsterWrap.classList.toggle("is-boss", !!monster.isBoss && !monster.isFinal);
      }
    }
  ...
  render() {
    ...
    this._text("playerClass", { night:"기사", mage:"마법사", archer:"궁수" }[p.type] || p.type);
    ...
    if (this.el.btnHeroUlt) {
      ...
      this.el.btnHeroUlt.textContent       = ready ? "⚡ 궁극기 READY!" : `⚡ 궁극기 ${gauge}%`;
      ...
      this.el.btnHeroUlt.style.animation   = ready ? "status-pulse 1s ease-in-out infinite" : "";
    }
    ...
    if (m) {
      ...
      if (this.el.monsterImg && m.img && !this.el.monsterImg.src.includes(m.img.split("/").pop())) {
  ...
  _showCutinImageThenVideo(imgSrc, videoSrc, title) {
    ...
    label.style.cssText = [
      "color:#FFD700", "font-size:2.4rem", "font-weight:700",
      "letter-spacing:.15em",
  ...
  _showCutinImage(imgSrc, title, duration) {
    ...
    const overlay = document.createElement("div");
    overlay.id = "cutinOverlay";
    overlay.style.cssText = "position:fixed;inset:0;z-index:999999;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;opacity:0;transition:opacity .3s ease;";

    const label = document.createElement("div");
    label.style.cssText = "color:#FFD700;font-size:2.4rem;font-weight:700;letter-spacing:.15em;text-shadow:0 0 30px #FFD700,0 0 60px #FFD70088;font-family:'Noto Serif KR',serif;margin-top:20px;text-align:center;padding:0 20px;";
    label.textContent = title;
    ...
    if (imgSrc) {
      const img = document.createElement("img");
      img.style.cssText = "max-width:92vw;max-height:78vh;object-fit:contain;border-radius:8px;box-shadow:0 0 80px rgba(255,215,0,0.35);";
      img.onload  = () => { overlay.appendChild(img); overlay.appendChild(label); fadeIn(); };
      img.onerror = () => { label.style.fontSize = "3.5rem"; label.style.marginTop = "0"; overlay.appendChild(label); fadeIn(); };
      img.src = imgSrc;
    } else {
