...
function generateDungeonMap(width = 25, height = 18) {
  ...
  for (let i = 0; i < enemyCount; i++) {
    place(TILE.ENEMY, { monsterId: enemyIds[Math.floor(Math.random() * enemyIds.length)] });
  }
...
class DungeonScene {
  ...
  init(canvas, dungeonType = "normal", savedState = null) {
    this.canvas      = canvas;
    ...
    if (savedState && savedState.mapData) {
      // ★ 전투 복귀 시 기존 맵 재사용 (objects 역직렬화)
      const sd = savedState.mapData;
      this.mapData = {
        map:     sd.map,
        rooms:   sd.rooms,
        objects: new Map(sd.objects), // Array → Map 복원
        startX:  sd.startX,
        startY:  sd.startY,
        width:   sd.width,
        height:  sd.height,
      ...
      // 새 맵 생성
      this.mapData = generateDungeonMap(25, 18);
    ...
    switch (obj.type) {
      ...
      case TILE.BOSS:
        ...
        if (this.dungeonType === "abyss") {
          // 심연 던전 → 마왕 직접 전투
          setTimeout(() => this.game.startBattle("demon", true), 100);
        } else {
          // ★ 일반 던전 → 수호자(guardian) 전투
          // 수호자 처치 후 _onGuardianDefeated()가 호출되어 마왕으로 이어짐
          this.guardianDefeated = false;
          setTimeout(() => this.game.startBattle("guardian", true), 100);
        }
        break;
  ...
  _render() {
    ...
    for (let ty = startTY; ty <= endTY; ty++) {
      for (let tx = startTX; tx <= endTX; tx++) {
        ...
        const isVisible = Math.abs(tx - this.playerX) <= 4 && Math.abs(ty - this.playerY) <= 4;
        ...
        if (obj && obj.type !== TILE.START) {
          ...
          if (icon) {
            ...
            if (obj.type === TILE.BOSS)       { ctx.shadowColor = "#ff3300"; ctx.shadowBlur = 20; }
            else if (obj.type === TILE.ENEMY) { ctx.shadowColor = "#ff6600"; ctx.shadowBlur = 14; }
            else if (obj.type === TILE.CHEST) { ctx.shadowColor = "#ffcc00"; ctx.shadowBlur = 12; }
            else if (obj.type === TILE.EXIT)  { ctx.shadowColor = "#00ffcc"; ctx.shadowBlur = 12; }
