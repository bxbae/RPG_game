...
<div class="town-body">
  <div class="town-map">
    ...
    <div class="town-locations">
      ...
      <button class="location-btn" id="tn-achv" style="border-color:#ffd700;color:#ffd700;">🏆 업적</button>
    </div>
  </div>
...
`;
  ...
  _bindEvents() {
    ...
    q("tn-achv")?.addEventListener("click", () => this._openAchievementModal());
  ...
  _openAchievementModal() {
    const old = document.getElementById("tnAchvModal");
    if (old) old.remove();

    const modal = document.createElement("div");
    modal.id = "tnAchvModal";
    modal.style.cssText = `
    ...
    const total = window.ACHIEVEMENTS?.length || 0;
    const done = Object.keys(achieved).length;

    modal.innerHTML = `
      <div style="background:#110d0f;border:2px solid var(--gold);border-radius:10px;
        padding:24px 28px;width:min(560px,94vw);max-height:85vh;overflow-y:auto;
        font-family:'Noto Serif KR',serif;">
        <div style="display:flex;justify-content:space-between;align-items:center;
          margin-bottom:16px;border-bottom:1px solid #3a2428;padding-bottom:12px;">
          <div>
            <span style="font-size:1.05rem;font-weight:700;color:var(--gold2);">🏆 업적</span>
            <span style="font-size:.7rem;color:var(--text-dim);margin-left:10px;">
              ${done} / ${total} 달성
            </span>
          </div>
          <button id="tnCloseAchv" style="background:transparent;border:1px solid #3a2428;
            color:var(--text-dim);padding:5px 14px;cursor:pointer;font-family:inherit;
            border-radius:4px;font-size:.8rem;">✕ 닫기</button>
        </div>
        <div id="tnAchvContent">
          ${window.achievementMgr?.renderUI(p) || window.AchievementManager ? new AchievementManager().renderUI(p) : "업적 시스템 로딩 중..."}
        </div>
      </div>`;

    document.body.appendChild(modal);
    modal.querySelector("#tnCloseAchv").onclick = () => modal.remove();
