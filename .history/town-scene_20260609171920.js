...
`;
  ...
  _bindEvents() {
    ...
    q("tn-abyss")  ?.addEventListener("click", () => {
      if (!g.player.abyssUnlocked) { g.showNarrative("🔒 마왕을 처치하면 해금됩니다.", 3000); return; }
      g.goToDungeon("abyss");
    });
    ...
    q("tnLearnAtk") ?.addEventListener("click", () => { this.game.learnSkill("attackBoost");   this._refreshSkillModal(); });
    q("tnLearnHp")  ?.addEventListener("click", () => { this.game.learnSkill("hpBoost");        this._refreshSkillModal(); });
    q("tnLearnCrit")?.addEventListener("click", () => { this.game.learnSkill("criticalBoost");  this._refreshSkillModal(); });
    q("tnLearnJob") ?.addEventListener("click", () => { this.game.learnJobSkill();              this._refreshSkillModal(); });
  ...
  _refreshBankScreen() {
    ...
    TOWN_STAGES.forEach((s2, i) => {
      ...
      const isCurrent = stage.level === s2.level - 1 || (!achieved && bank.totalInvested < s2.minInvest && (i === 1 || bank.totalInvested >= TOWN_STAGES[i-1].minInvest));

      const prevMin  = TOWN_STAGES[i - 1].minInvest;
      const rangeMax = s2.minInvest - prevMin;
      const rangeNow = Math.max(0, Math.min(bank.totalInvested - prevMin, rangeMax));
        ...
        <div class="invest-bar-track">
          <div class="invest-bar-fill" style="width:${achieved ? 100 : pct}%;
            background:${achieved
              ...
              : `linear-gradient(90deg,#442200,${s2.color}88)`};"></div>
        </div>
  ...
  _investTown(amount) {
    ...
    INVEST_REWARDS.forEach(r => {
      if (bank.totalInvested >= r.minInvest && !bank.milestones.includes(r.minInvest)) {
        bank.milestones.push(r.minInvest);
        if (r.atkBonus) p.bonusAttack = (p.bonusAttack || 0) + r.atkBonus;
        if (r.hpBonus)  { p.maxHp += r.hpBonus; p.hp = Math.min(p.hp + r.hpBonus, p.maxHp + p.bonusHp); }
  ...
  render() {
    ...
    const gc = { normal:"#b8a888", uncommon:"#55cc55", rare:"#4898d8", epic:"#bb66ff", legend:"#d8a820" };
    const fmtEq = it => it
      ? `<span style="color:${gc[it.class]||"#b8a888"}">+${it.enhance||0} ${it.name}</span>`
      : `<span style="color:#504040;">없음</span>`;

    const we = q("tnWeapon"); if (we) we.innerHTML = fmtEq(p.equipment.weapon);
    const he = q("tnHelmet"); if (he) he.innerHTML = fmtEq(p.equipment.helmet);
    const ae = q("tnArmor");  if (ae) ae.innerHTML = fmtEq(p.equipment.armor);

    const cs = q("tnCompEquip");
    if (cs) {
      if (p.party) {
        cs.style.display = "block";
        q("tnCompEquipName").textContent = `⚔ ${PARTY_MEMBERS[p.party]?.name} 장착`;
    ...
    if (p.quest) {
      ...
      s("tnQuestTitle","없음"); s("tnQuestProg",""); s("tnQuestReward","");
    }
    ...
    if (next) {
      ...
      const pct = Math.floor(((bank.totalInvested - prevMin) / (next.minInvest - prevMin)) * 100);
      const bar = q("tnTownBar"); if (bar) bar.style.width = `${pct}%`;
    } else {
      s("tnTownNextGoal", "✅ 최고 단계!");
      const bar = q("tnTownBar"); if (bar) bar.style.width = "100%";
    }
  ...
  _renderInventory() {
    ...
    if (!c) return;
    c.innerHTML = "";
    const grade = { normal:{color:"#b8a888"}, uncommon:{color:"#55cc55"}, rare:{color:"#4898d8"}, epic:{color:"#bb66ff"}, legend:{color:"#d8a820"} };
    ...
    p.inventory.forEach((item, idx) => {
      ...
      const stat = item.type === "weapon" ? `ATK+${item.attack}` : item.type === "potion" ? `회복` : `DEF+${item.defense}`;
      row.innerHTML = `
      ...
      if (item.type !== "potion") {
        ...
        eq.className = "inv-btn"; eq.textContent = "장착";
        eq.addEventListener("click", () => { this.game.itemManager.equip(this.game, idx, false); this.render(); });
        row.appendChild(eq);
        if (p.party) {
          const eqC = document.createElement("button");
          eqC.className = "inv-btn"; eqC.textContent = "동료";
          eqC.style.cssText = "border-color:#4444cc;color:#8888ff;";
          eqC.addEventListener("click", () => { this.game.itemManager.equip(this.game, idx, true); this.render(); });
          row.appendChild(eqC);
        }
      }
      const del = document.createElement("button");
      del.className = "inv-btn del"; del.textContent = "❌";
      del.addEventListener("click", () => { this.game.itemManager.remove(this.game, idx); this.render(); });
      row.appendChild(del);
      c.appendChild(row);
    });
  }
  ...
  _openSkillModal() {
    ...
    s("tnJobSkillName", { night:"⚔ 회전베기", mage:"🔮 매직볼", archer:"🏹 속사" }[p.type]);
  ...
  _renderPassiveSkills() {
    ...
    const allPassives = {
      ...
      mage: [
        { id:"mana_surge",  name:"💥 마나 폭발",  desc:["크리+20%","+40%","+65%"],       spCost:[2,3,4] },
        { id:"spell_echo",  name:"🔮 마법 반향",  desc:["25%추가15","30%추가30","40%추가50"], spCost:[2,3,4] },
        { id:"arcane_ward", name:"🌀 비전 보호막",desc:["피해-8%","-15%","-22%"],         spCost:[2,3,4] },
      ],
      archer: [
        { id:"eagle_eye",  name:"🦅 매의 눈",  desc:["크리+8%","+16%","+26%"],               spCost:[2,3,4] },
        { id:"swift_feet", name:"💨 신속",      desc:["피해-10%(20%)","-15%(30%)","-20%(40%)"],spCost:[2,3,4] },
  ...
  _openSmithModal() {
    ...
    if (enhEl) {
      ...
      p.inventory.forEach((item, idx) => {
        ...
        btn.addEventListener("click", () => { this.game.itemManager.enhance(this.game, idx); this._openSmithModal(); });
    ...
    if (sellEl) {
      ...
      p.inventory.forEach((item, idx) => {
        ...
        btn.addEventListener("click", () => { this.game.itemManager.sellToBlacksmith(this.game, idx, price); this._openSmithModal(); this.render(); });
  ...
  _getPortrait(type) {
    const map = {
      night:  "images/portrait_Night.png",
      mage:   "images/portrait_mage.png",
  ...
  _getPartyName(party) {
    const map = {
      healer: "리온 (힐러)", tanker: "카인 (탱커)",
      dealer: "카르나 (딜러)", mage_party: "엘린 (마법사)",
      archer: "아리아 (궁수)",
    };
  ...
  _renderSaveSlots(tab) {
    ...
    slots.forEach(slot => {
      ...
      if (!slot.empty && slot.data?.player) {
        ...
        portrait.style.cssText = "width:56px;height:56px;border-radius:6px;object-fit:cover;border:2px solid #4a2e38;flex-shrink:0;";
        ...
        if (sp.party) {
          ...
          cPortrait.style.cssText = "width:44px;height:44px;border-radius:6px;object-fit:cover;border:2px solid #3a2848;flex-shrink:0;margin-left:-24px;margin-top:12px;";
          cPortrait.onerror = () => { cPortrait.style.display = "none"; };
  ...
  _doLoad(slotIndex) {
    ...
    if (!data) { this.game.showNarrative("⚠ 데이터 없음", 2000); return; }

    const player = this.game.saveManager.hydrate(data.player);
    if (!player) { this.game.showNarrative("⚠ 데이터 손상", 2000); return; }
  ...
  _openPartyModal() {
    ...
    Object.entries(PARTY_MEMBERS).forEach(([key, mem]) => {
      ...
      btn.addEventListener("click", () => { this.game.selectParty(key); modal.style.display = "none"; this.render(); });
